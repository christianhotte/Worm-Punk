...
public class NewGrapplerController : PlayerEquipment
{
    ...
    public void HookedObstacle()
    {
        ...
        //Effects:
        if (settings.hitSound != null) audioSource.PlayOneShot(settings.hitSound, PlayerPrefs.GetFloat("SFXVolume", 0.5f)); //Play sound effect
    ...
    public void HookedPlayer()
    {
        ...
        if (settings.playerHitSound != null) audioSource.PlayOneShot(settings.playerHitSound, PlayerPrefs.GetFloat("SFXVolume", 0.5f)); //Play sound effect
        SendHapticImpulse(settings.hitHaptics);                                                //Play haptic impulse
    }
    /// <summary>
    ...
    {
        if (settings.releaseSound != null) audioSource.PlayOneShot(settings.releaseSound, PlayerPrefs.GetFloat("SFXVolume", 0.5f)); //Play sound effect
    ...
    public void Bounced()
    {
        if (settings.bounceSound != null) audioSource.PlayOneShot(settings.bounceSound, PlayerPrefs.GetFloat("SFXVolume", 0.5f)); //Play sound effect
        SendHapticImpulse(settings.releaseHaptics);                                      //Play haptic impulse
    }
    ...
    private void Launch()
    {
        ...
        if (Vector3.Angle(forwardHandVel, hand.forward) < 90 && forwardHandVel.magnitude > settings.punchWhipSpeed) //Player is punching hard/fast enough to trigger instant grab
        {
            ...
            if (settings.whipSound != null) audioSource.PlayOneShot(settings.whipSound, PlayerPrefs.GetFloat("SFXVolume", 0.5f)); //Play sound effect
        }
        else //Normal launch effects
        {
            if (settings.launchSound != null) audioSource.PlayOneShot(settings.launchSound, PlayerPrefs.GetFloat("SFXVolume", 0.5f)); //Play sound effect
        }
